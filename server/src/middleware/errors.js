...
export function errorHandler(error, _req, res, _next) {
  const uploadError = error.name === 'MulterError' || error.message === 'Only PDF files are allowed';
  const status = error.status || (uploadError ? 400 : error.name === 'CastError' ? 404 : 500);
